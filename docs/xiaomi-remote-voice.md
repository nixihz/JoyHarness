...
- 小米遥控器可与 DualSense 等游戏手柄同时使用。BLE 语音服务就绪后会预建输出引擎，
  正常语音会话之间保持与 Joy Harness 虚拟麦克风的绑定，避免每次按键都重新配置 CoreAudio。
  带音频端点的手柄连接或断开仍可能触发系统设备重配置；输出引擎会恢复绑定并重试当前语音包。
- 输出缓冲最多保留 8,000 个采样（0.5 秒），且只向 CoreAudio 预送少量音频包；遥控器与
  CoreAudio 时钟短暂漂移使缓冲到顶时，从尚未播放的队首裁掉最旧样本，保留最新讲话。
  消费恢复后继续当前会话，不会截断剩余讲话。HID 松键与 BLE 停流可按任意顺序到达；
