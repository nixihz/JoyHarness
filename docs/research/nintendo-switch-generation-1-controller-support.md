...
2. 建立 Joy-Con 逻辑输入层：上层继续消费统一按钮、摇杆和按压事件，底层可按字段合并 `GameController` 与 HID。单支由 `GameController` 提供摇杆、面键、摇杆按下和 menu，由原始 HID 区分四颗实体肩键；分开枚举的双支同样用 HID 的外侧位确定 L/ZL/R/ZR。每个规范化字段只有一个所有者，模糊的 GC shoulder 字段被替换或抑制；同侧多个 HID endpoint 若状态一致则合并，只有状态冲突时才暂不暴露对应肩键。
